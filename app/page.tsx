// app/page.tsx (without Cloudflare Turnstile - with Rate Limiting)
"use client";
...
const STORAGE_KEY = "redpulse_login_attempts";
...
function getLoginAttempts(): LoginAttemptData {
  ...
  try {
    ...
    if (stored) {
      const data = JSON.parse(stored) as LoginAttemptData;
...
function saveLoginAttempts(data: LoginAttemptData) {
...
export default function HomePage() {
  ...
  // =========================================================
  // RATE LIMITING: Initialize from localStorage
  // =========================================================
  useEffect(() => {
    ...
    if (data.lockedUntil) {
      setCooldownRemaining(data.lockedUntil - Date.now());
    }
  }, []);

  // =========================================================
  // RATE LIMITING: Cooldown countdown timer
  // =========================================================
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        setLoginError("");
        saveLoginAttempts({ attempts: 0, lockedUntil: null });
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    if (lockedUntil && Date.now() < lockedUntil) {
      setLoginError(
        `Too many failed attempts. Please wait ${formatCooldown(lockedUntil - Date.now())} before trying again.`
      );
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        if (newAttempts >= MAX_LOGIN_ATTEMPTS) {
          ...
          const lockUntilTime = Date.now() + LOCKOUT_DURATION_MS;
          setFailedAttempts(newAttempts);
          setLockedUntil(lockUntilTime);
          setCooldownRemaining(LOCKOUT_DURATION_MS);
          saveLoginAttempts({ attempts: newAttempts, lockedUntil: lockUntilTime });
          setLoginError(`Too many failed attempts. Account locked for 3 minutes.`);
        } else {
          const remaining = MAX_LOGIN_ATTEMPTS - newAttempts;
          setFailedAttempts(newAttempts);
          saveLoginAttempts({ attempts: newAttempts, lockedUntil: null });
          setLoginError(
            data.error ||
  ...
  return (
    <main className="min-h-screen bg-[#020303] text-white overflow-x-hidden">
      ...
      {/* =========================================================
          LOGIN MODAL WITH RATE LIMITING (No Turnstile)
      ========================================================= */}
      <AnimatePresence>
        {showLogin && (
          <motion.div
          ...
          >
            ...
            >
              <div className="p-5 sm:p-7">
                ...
                {isLockedOut && (
                  <div className="mb-5 p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-start gap-2.5">
                    <AlertTriangle className="w-5 h-5 text-amber-500 flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="text-sm font-semibold text-amber-500">Account Temporarily Locked</p>
                ...
                {!isLockedOut && failedAttempts > 0 && (
                  <div className="mb-5 p-3 rounded-xl bg-red-500/10 border border-red-500/20 flex items-start gap-2.5">
                    ...
                    <p className="text-xs text-red-400">
                      {MAX_LOGIN_ATTEMPTS - failedAttempts} attempt
                      {MAX_LOGIN_ATTEMPTS - failedAttempts === 1 ? "" : "s"} remaining before lockout
                    </p>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="relative mt-1 sm:mt-2">
                      ...
                      <input
                        ...
                        className="w-full bg-white/[0.04] border border-white/10 rounded-xl py-2.5 sm:py-3 pl-9 sm:pl-10 pr-3 sm:pr-4 text-sm outline-none focus:border-red-500 transition disabled:opacity-50"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative mt-1 sm:mt-2">
                      ...
                      <input
                        ...
                        className="w-full bg-white/[0.04] border border-white/10 rounded-xl py-2.5 sm:py-3 pl-9 sm:pl-10 pr-10 sm:pr-11 text-sm outline-none focus:border-red-500 transition disabled:opacity-50"
                      />
                      <button
                        type="button"
      ...
      {/* =========================================================
          FORGOT PASSWORD MODAL (No Turnstile)
      ========================================================= */}
      <AnimatePresence>
        {showForgotPassword && (
          <motion.div
